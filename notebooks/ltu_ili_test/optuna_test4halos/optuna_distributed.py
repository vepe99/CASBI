...
def objective(trial):
    ...
    hidden_features = trial.suggest_categorical('hidden_features', [50, 70, 100])
    num_transforms = trial.suggest_categorical('num_transforms', [10, 15, 20])
    learning_rate = trial.suggest_categorical('learning_rate', [5e-5, 1e-4]) #suggest_loguniform('learning_rate', 1e-5, 1e-4)
