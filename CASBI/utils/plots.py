...
"""
def custom_kde_plot(test_df: pd.DataFrame, df_sample:pd.DataFrame, model:torch.nn.Module, nll: float, kl_mean: float, js_mean:float, D_mean:float, levels=5,):
    """
    ...
    for i, data_type in enumerate(['test', 'sample']):
        ...
        if data_type == 'test':
            ...
            if isinstance(model, NF_condGLOW): 
                flow_pdf  = model.get_pdf(test_df.values[:, :2].astype(float), test_df[other_columns].values[0, 2:].astype(float))
            elif isinstance(model, FreeFormFlow):
                _, flow_logprob = model.log_prob(test_df.values[:, :2].astype(float), test_df[other_columns].values[0, 2:].astype(float))
