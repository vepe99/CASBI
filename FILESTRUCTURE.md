...
* Python source files:
  * The Python package source files are located in the `CASBI` directory.
  * The file `CASBI.__main__.py` defines a command line interface that
    is accessible both as the command `CASBI` and via
    `python -m CASBI`.
  * `tests/test_cli.py` contains rudimentary testing of this CLI.
  * `tests/test_CASBI.py` contains the unit tests for the package.
  * `tests/conftest.py` contains testing setup and configuration for `pytest`
  * The `notebooks` directory contains an example Jupyter notebook on how to use `CASBI`.
